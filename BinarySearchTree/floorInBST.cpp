...
int floorInBST (Node *root, int data) { //Time Complexity: O(height), Auxiliary Space; O(1)
    Node *curr = root;
    int result = -1;
...
int main() {
    ...
    inorder(root);
    int data = 11;
    std::cout << std::endl << "The floor of " << data << " is " << floorInBST(root,data)->data << std::endl;
    free(root);
    return 0;
}