...
void bottomViewHelper(Node *root, std::map<int, std::vector<int>> &map) {
    if (root == nullptr)
        return;
...
void bottomViewOfBinaryTree(Node *root) {
    ...
    bottomViewHelper(root, map);

    for (std::pair<int, std::vector<int>> i: map) { //Just printing the last element of each vertical
...
//--------------Efficient Solution-------------------
void bottomViewHelper(Node *root, std::map<int, int> &map) {
    if (root == nullptr)
        return;
...
void bottomViewOfBinaryTree(Node *root) { //Time Complexity: O(N), Auxiliary Space: O(n)
    std::map<int, int> map;
    bottomViewHelper(root, map);
